...
// Create a input label for multi inputs
function promptCreateInputLabel(label, useHtmlLabel) {
...
function promptCreateMultiInput(parentElement) {
	let el, maxLabelCharacters = 0, labels = [];
	for (const option of promptOptions.multiInputOptions) {
    const {label, useHtmlLabel} = option;
		el = promptCreateInput(option);
		el.setAttribute("id", "data");

    // If we are adding a individual input label create a field to wrap the label and the input
    if (label) {
      const field = document.createElement("div")
      const lel = promptCreateInputLabel(label, useHtmlLabel);

      // we keep track of the labels so we can apply a consistent width to all labels at the end
      labels.push(lel);
      if (label.length > maxLabelCharacters) maxLabelCharacters = label.length;

      field.classList.add("multinput-field");
      field.append(lel);
      field.append(el);
      parentElement.append(field);
    } else {
      parentElement.append(el);
    }
	}

  labels.forEach(label => label.style.minWidth = `${maxLabelCharacters}ch`)
	return el;
}
